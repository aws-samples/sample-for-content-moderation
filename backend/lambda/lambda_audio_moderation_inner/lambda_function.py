    ...
    if original_content:
        ...
        if save_flag == "0":
            ...
            if state == 2:
                ...
                save_and_push_message(task_id, user_id, s3_file_path, s3_read_path, original_content, start_time,
                                      end_time, confidence,
                                      tag,
                                      des, "audio",
                                      state)
            ...
            s3_read_path = s3_clients.get_presigned_url(MODERATION_BUCKET_NAME, s3_file_path,
                                                        S3_FILE_READABLE_EXPIRATION_TIME)
            save_and_push_message(task_id, user_id, s3_file_path, s3_read_path, original_content, start_time, end_time,
                                  confidence,
                                  tag,
                                  des, "audio",
                                  state)
...
if __name__ == '__main__':
    ...
    event['Records'][0]['body'] = json.dumps({
        "user_id": "lee",
        "task_id": "12341234",
        "s3_file": "audio/2bd95247-c597-4f18-9b59-f23937a3b658.wav",
        "save_flag": "1",
