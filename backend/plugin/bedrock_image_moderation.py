...
from base.plugin_interface import ImageModeration
from config import IMG_MODEL_ID, IMG_PROMPT_0325_CN, \
    SYSTEM_0325_NOVA_VIDEO_CN, VIDEO_PROMPT_0325_CN, \
    IMG_NOVA_SYSTEM_PROMPT_1_EN, IMG_CLAUDE_SYSTEM_PROMPT
from tools.bedrock_img_tool import invoke_claude, batch_invoke_nova, batch_invoke_claude
from tools.bedrock_img_tool import invoke_nova
...
class BedrockImageModeration(ImageModeration):
...
def check_images(image_path_arr: list[str]) -> list[any]:
    ...
    for file_path in image_path_arr:
        ...
        if blank_flag is False:
            ...
            logger.info(f"{str(file_path)} is blank img")
